...
CI_PROJECT_DIR=${CI_PROJECT_DIR:-$(mktemp -d)/nouveau}
...
/usr/bin/wget -O $TMP_DIR/nouveau.tar.gz \
              https://${MINIO_HOST}/git-cache/${FDO_UPSTREAM_REPO}/nouveau.tar.gz
...
tar xzf "$TMP_DIR/nouveau.tar.gz" -C "$CI_PROJECT_DIR"
rm -rf "$TMP_DIR"
